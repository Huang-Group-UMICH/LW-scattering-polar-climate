...
!      use parkind, only : jpim, jprb 
      use rrlw_vsn
! yihsuan comment      use mcica_subcol_gen_lw, only: mcica_subcol_lw
...
!      use rrtmg_lw_init, only: rrtmg_lw_ini
      ...
      !use rrtmg_lw_rtrnmc, only: rtrnmc               ! yihsuan add, call rtrnmc and rtr2function separately
      !use rrtmg_lw_rtr2function, only: rtr2function   !
...
!------------------------------------------------------------------
      ...
      subroutine rrtmg_lw &
            (lchnk   ,ncol    ,nlay    ,icld    ,  lw_cloud_scat,                 &  ! yihsuan add input logical, lw_cloud_scat
             play    ,plev    ,tlay    ,tlev    ,tsfc    ,h2ovmr  , &
             o3vmr   ,co2vmr  ,ch4vmr  ,o2vmr   ,n2ovmr  ,&
             cfc11vmr,cfc12vmr, &
             cfc22vmr,ccl4vmr ,emis    ,inflglw ,iceflglw,liqflglw, &
             !yihsuan cancel cldfmcl ,taucmcl ,ciwpmcl ,clwpmcl ,reicmcl ,relqmcl , &
...
! New variables in TAMU Ice Optics/Scattering scheme
      logical, intent(in)       :: lw_cloud_scat       ! flag to control cloud LW scattering
      ...
      real(kind=r8) :: xmomcmc(0:1,ngptlw,nlay)  ! LOCAL, cloud phase function expansion coefficient
...
!  other input parameters.  
         ...
         call inatm (iplon, nlay, icld, iaer, &
              ...
              cfc22vmr, ccl4vmr, emis, inflglw, iceflglw, liqflglw, &
              !yihsuan cancel cldfmcl, taucmcl, ciwpmcl, clwpmcl, reicmcl, relqmcl, tauaer, &
...
! For McICA, RTRNMC is called for clear and cloudy calculations.
         ...
         if (lw_cloud_scat) then
!flag11
!do k = 1, nlay
!  if (taucmc(1,k) .gt. 0.) then             
!    print*,'gg',k,taucmc(:,k)
!  endif
!enddo

           ! set atm OD = 0 & surface emission = 0
           !taut = 0._r8
           !plankbnd = 0._r8
           ...
           !set atm OD = 0 in non-cloud layers
           !do k=1,nlay
           !  if (taucmc(1,k) .le. 0.) then
           !    taut(k,:) = 0._r8
           !  end if
           !end do

           !set atm OD = 0 in non-cloud layers
           !set no gas in the cloud layer
           !do k=1,nlay
           !  if (taucmc(1,k) .gt. 0.) then
           !    taut(k,:) = taucmc(:,k)
           !    planklev(k-1,:) = 0._r8 
           ...
           !set atm OD = 0 in non-cloud layers, no gas effect the cloud
           !layer, 
           !do k=1,nlay
           !  if (taucmc(1,k) .gt. 0.) then
           !    taut(k,:) = taucmc(:,k)
           ...
           ! Compute R.A.T
           !*********************************************
           !set atm NO absorption, emission
           !set cloud NO emission
           !ONLY: cloud absorption, scattering & surface emission
           !taut = 0._r8
           !planklev = 0._r8
           !planklay = 0._r8

           !set atm NO absorption, emission
           !ONLY: cloud absorption, scattering emission & surface emission
           !taut = 0._r8

           !set atm NO absorption, emission
           !set surface NO emission
           !ONLY: cloud absorption, scattering, emission
           !taut = 0._r8
           !plankbnd = 0._r8
           !*********************************************

           !set atm NO absorption, emission EXCEPT a layer above the cloud
           !set surface NO emission
           !ONLY: cloud absorption, scattering emission & a layer of atm emission
           !plankbnd = 0._r8
           !do k=1,nlay
           !  if (k.eq.15) then
           !    ttt1 = 0._r8
        ...
        !  if (taucmc(1,k) .gt. 0.) then             
            ...
            !do ig = 1, ngptlw
            !  print*,'k,ig',k,ig
            !  print*,'taut=',taut(k,ig),',taucmc=',taucmc(ig,k)
            !end do
        !  endif
        !enddo
