...
! CPKuo@TAMU 
      real(kind=r8), intent(out) :: ssacloud(:,:)     ! cloud single scattering albedo
                                                      !    Dimensions:
                                                      !    (nlayers,nbndlw)
      real(kind=r8), intent(out) :: xmomcloud(0:,:,:)  ! cloud phase function expansion coefficient
                                                      !    Dimensions:
                                                      !    (moments,nlayers,nbndlw)
...
! CPKuo@TAMU 
      real(kind=r8) :: invrad                         ! inverse of effective particle size
      real(kind=r8) :: diaice                         ! cloud ice effective diameter (microns)
      real(kind=r8) :: asycloud                       ! ice asymmetry factor
      real(kind=r8) :: fpeak                          ! fraction of forward peak
      real(kind=r8) :: extcoice(nbndlw)               ! ice extinction coefficients
      real(kind=r8) :: ssacoice(nbndlw)               ! ice single-scattering albedo
      real(kind=r8) :: asycoice(nbndlw)               ! ice asymmetry factor
...
!                    140.0 micron.
!       ICEFLAG = 4: The ice effective diameter (de) is input and the
!                    optical properties are calculated by fitted
!                    polynomial. Valid range of values for de are
!                    between 3.0 and 370.0 micron. ! CPKuo@TAMU
...
! CPKuo@TAMU
! Add iceflag=4 option. This option is for MODIS collection 6 ice model
! parameterizations.
! Use iceflag=4 option for ice particle effective diameter from 3.0 and
! 370.0 microns
! and use iceflag=0 option for ice particles greater than 370.0 microns.
! *** NOTE: Transition between two methods has not been smoothed. 
  
               elseif (iceflag .eq. 4) then
                  diaice = radice * 2.0_r8
                  if (diaice .lt. 3.0_r8) &
                     stop 'ICE EFFECTIVE DIAMETER OUT OF BOUNDS'
                  if (diaice .ge. 3.0_r8 .and. diaice .le. 370.0_r8) then
                     ncbands = 16
                     invrad = 1.0/diaice - 0.04_r8
                     do ib = 1, ncbands
                        if (diaice .ge. 25.0_r8) then
                           abscoice(ib) = ((absice4(1,1,ib) * invrad + &
                                           absice4(1,2,ib)) * invrad + &
                                           absice4(1,3,ib)) * invrad + &
                                           absice4(1,4,ib)
                        else
                           abscoice(ib) = ((((((absice4(2,1,ib) * invrad + &
                                               absice4(2,2,ib)) * invrad + &
                                               absice4(2,3,ib)) * invrad + &
                                               absice4(2,4,ib)) * invrad + &
                                               absice4(2,5,ib)) * invrad + &
                                               absice4(2,6,ib)) * invrad + &
                                               absice4(2,7,ib)) * invrad + &
                                               absice4(2,8,ib)
                        endif
                     enddo
                     icepat = 2
                  elseif (diaice .gt. 370.0_r8) then
                     do ib = 1, ncbands
                        abscoice(ib) = absice0(1) + absice0(2)/radice
                     enddo
                     icepat = 0
